    ...
    public function adicionar()
    {
        ...
        if (isset($_POST['alcunha'])) {
            ...
            if ($lid) {
                $msg = '';
                if (isset($_POST['app_'])) {
                    if ($_POST['servidor_app'] != '')
                        $insert_app = $d->addBanco($lid, '', $_POST['alcunha'], $_POST['servidor_app']);
                    if (!$insert_app)
                        $msg .= 'Falha ao inserir app_. ';
                }
                if (isset($_POST['dlv_'])) {
                    if ($_POST['servidor_dlv'] != '')
                        $insert_dlv = $d->addBanco($lid, 'dlv_', $_POST['alcunha'], $_POST['servidor_dlv']);
                    if (!$insert_dlv)
                        $msg .= 'Falha ao inserir dlv_. ';
                }
                if (isset($_POST['dwh_'])) {
                    if ($_POST['servidor_dwh'] != '')
                        $insert_dwh = $d->addBanco($lid, 'dwh_', $_POST['alcunha'], $_POST['servidor_dwh']);
                    if (!$insert_dwh)
                        $msg .= 'Falha ao inserir dwh_. ';
                }
                if (isset($_POST['wap_'])) {
                    if ($_POST['servidor_wap'] != '')
                        $insert_wap = $d->addBanco($lid, 'wap_', $_POST['alcunha'], $_POST['servidor_wap']);
                    if (!$insert_wap)
